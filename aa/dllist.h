...
/*
        ...
        <>DLList dll_nil (DLList l) : Trả về một con trỏ tới nút nil (sentinel) cho danh sách. 
Bạn có thể nghĩ nil là một nút không có giá trị, bắt đầu và kết thúc danh sách. 
Vì l trỏ đến nút sentinel, dll_nil trả về l . Bạn không cần gọi dll_nil () để truy cập nút sentinel. 
Bạn chỉ có thể sử dụng l , mặc dù nó làm cho mã của bạn dễ đọc hơn nếu bạn sử dụng dll_nil () .
        <>DLList dll_first (DLList l) : Trả về một con trỏ tới nút đầu tiên trong danh sách. 
Nếu danh sách trống, cái này trả về sentinel. Như với dll_nil () , bạn không cần gọi dll_first (l) - bạn chỉ có thể sử dụng l-> flink .
        <>DLList dll_last (DLList l) : Trả về một con trỏ đến nút cuối cùng trong danh sách. Nếu danh sách trống, cái này trả về sentinel. 
Như với dll_nil () , bạn không cần gọi dll_last (l) - bạn chỉ cần sử dụng l-> nháy .
        <>DLList dll_next (DLList n) : Trả về một con trỏ tới nút tiếp theo trong danh sách sau n . 
Nếu n là nút cuối cùng trong danh sách, thì dll_next (n) trả về sentinel. 
Như với dll_first () , bạn không cần gọi dll_next (n) - bạn chỉ có thể sử dụng n-> flink .
        <>DLList dll_prev (DLList n) : Trả về một con trỏ tới nút trước đó trong danh sách trước n . 
Nếu n là nút đầu tiên trong danh sách, thì dll_prev (n) trả về sentinel. 
Như với dll_last () , bạn không cần gọi dll_prev (n) - bạn chỉ cần sử dụng n-> nháy .
